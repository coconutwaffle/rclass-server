...
CREATE TABLE account (
    ...
    account_id VARCHAR(255) UNIQUE, -- guest인 경우 NULL 강제
    name TEXT NOT NULL,
...
CREATE TABLE classes (
    ...
    creator_id UUID NOT NULL REFERENCES account(id) ON DELETE CASCADE,
...
CREATE TABLE rooms (
    ...
    class_id UUID REFERENCES classes(class_id) ON DELETE SET NULL, -- 수업이 삭제되면 null
...
CREATE TABLE room_attendees (
    ...
    attendee_id UUID NOT NULL REFERENCES account(id) ON DELETE CASCADE,
...
CREATE TABLE attendance_logs (
    room_id UUID NOT NULL REFERENCES rooms(room_id) ON DELETE CASCADE,  -- 방 삭제 시 출결 로그 제거
    attendee_id UUID NOT NULL REFERENCES account(id) ON DELETE CASCADE, -- 계정 삭제 시 관련 로그 제거

    status attendance_status DEFAULT 'present',  -- 출석 상태
    reason TEXT DEFAULT '',                      -- 지각/조퇴/결석 사유 등
    guest BOOLEAN DEFAULT FALSE,                 -- 게스트 여부

    log_data JSONB NOT NULL,                     -- {"summary": {...}, "per_block": [...]}
...
CREATE TABLE chat_target (
    chat_id UUID NOT NULL REFERENCES chat_logs(chat_id) ON DELETE CASCADE,
    target_id UUID NOT NULL REFERENCES account(id) ON DELETE CASCADE,
